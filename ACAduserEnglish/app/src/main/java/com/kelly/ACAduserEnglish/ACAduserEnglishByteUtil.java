...
public class ACAduserEnglishByteUtil {
    public static String printByteArrayHex(byte[] info, int offset, int length) {
        ...
        for (int i = offset; i < offset + length; i++) {
            ...
            String hexStr = Integer.toHexString((info[i] < 0 ? info[i] + 256 : info[i]) == 1 ? 1 : 0).toUpperCase();
            if (hexStr.length() == 1) {
                sb.append("0" + hexStr);
            } else {
                sb.append(hexStr);
        ...
        if (sw == null || !sw.equals("uo")) {
            if (sw == null || !sw.equals("h")) {
                if (sw == null || !sw.equals("a")) {
                    ...
                    for (int i = offset; i < offset + pos + 1; i++) {
                ...
                for (int i2 = offset; i2 < offset + pos + 1; i2++) {
                    info[i2] = Integer.valueOf(result.substring((i2 - offset) * 2, ((i2 - offset) * 2) + 2), 16).intValue();
                }
                return 1;
            }
        } else if (pro == 0) {
            ...
            for (int i3 = offset; i3 < offset + pos + 1; i3++) {
                info[i3] = finaltempInt / ((int) Math.pow(256.0d, (double) (pos - (i3 - offset))));
                if (i3 < offset + pos) {
                    finaltempInt -= info[i3] * ((int) Math.pow(256.0d, (double) (pos - (i3 - offset))));
                }
            }
            return 1;
        }
    ...
    public static int get_Index(int textview_count, int j, int mode) {
        ...
        return mode == 0 ? colIndex : rowIndex;
    }
